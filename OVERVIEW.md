...
#### 拉长到 16 epoch:有效优化只有 1 步,其余全是膨胀

把 OfficeQA 训练拉到 **16 epoch / 32 步**(忠实沿用论文配置,只改 epoch 数),用**等距横轴**(不做论文 1/2/4/8/16 的翻倍压缩):

![OfficeQA 16-epoch step trends](./skillopt-assets/officeqa-step-trends-16ep.png)

- **32 步里只有 1 步被 gate 接受**(accept=1, reject=30, skip=1)。
- **Selection best 在 step 4(epoch 2)跳到 0.792 后,28 步纹丝不动**(其余步因严格 `>` 全被拒;step 32 打平 0.792 也被拒)。
- **skill 长度 887 → 18,775 字符(21×)**,靠 epoch 末 slow_update 强制注入持续膨胀,与那条水平的 best 线**完全脱钩**(中途的下凹是 meta-skill 压缩)。
- **测试集(172)印证**:baseline `0.599` → best_skill(step 4)`0.715` → 最终(step 32)`0.715`。
  **全部 +0.116 的提升在 step 4 就拿满,之后 28 步零增益**;代价是 **21× 的 skill 体积、7.6 小时、1.06 亿 token**。
  且 final == best,说明膨胀对精度**近似中性**(印证下方研究记录 ③)。

结论很硬:**这套"self-evolving"在 OfficeQA 上等价于"agent 反思一次、更新一次 skill,之后 14 个 epoch 颗粒无收",而论文的翻倍横轴在视觉上把这条长平/膨胀的尾巴压没了。** 两个机制问题见下。

#### 两个机制问题(已观察,待深入)

1. **epoch-level 强制更新有 bug 性质**:`run_slow_update` 默认 `slow_update_gate_with_selection=False` → 生成内容**不过 EM gate、force-accept**(`trainer.py:1894-1901`,甚至把上一轮分数当它的分),是膨胀的直接来源。
2. **首次 accept 后再无真正更新**:候选 edit 长期打不过早期 best。候选主因:(a) selection 集仅 24 题、rollout 采样有噪 → gate 误拒;(b) 严格 `>` 早早锁死 best;(c) 膨胀的 base 反噬后续 reflect。

> 研究记录(1/2/3 优先级低,先存档):①给 slow_update 加 EM gate 的 A/B;②同 skill 重复 rollout 量化 gate 噪声/误拒率;③用 `--versions-dir` 测 e16 各版本在测试集上是否随长度掉分(初步印象:对精度近似无害、但 token 成本翻倍)。**优先做方向 4(下节)。**
