...
- **Commits/PRs go to the personal fork `litaohz`** via the `commit-to-personal-repo`
  ...
  **not** `gh pr ...` (token lacks `read:org` for GraphQL). Always add **both** AI
  co-author trailers so they appear as contributors:
  - `Co-authored-by: Copilot <223556219+Copilot@users.noreply.github.com>`
  - `Co-authored-by: Claude <noreply@anthropic.com>`
  (Copilot maps to the official GitHub bot account and shows as a contributor; the Claude
  trailer is recorded on every commit, though GitHub may not list it as a contributor
  unless that email is linked to a GitHub account.)
