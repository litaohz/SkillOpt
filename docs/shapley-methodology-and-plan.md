...
### P1 — 误差棒 / 收敛（对应 Q1、对齐参考实现的收敛判据）
- [ ] `skill_combo.py` 支持 **多 seed**（或把 perms 拆成 batch），对每个 φ_i 报
      **均值 ± 标准误**；输出到 `shapley.csv` 增列 `se`、`n_samples`。
- [ ] 加大 perms（先 20，再看收敛）复跑 OfficeQA e16，复用现有 `eval_cache.jsonl`
      （已有 48 个子集，多数会命中缓存）。
- [ ] 加一个 **efficiency 自检打印**（ΣShapley vs full−empty）作为回归守卫。
- **验收**：中间项（尤其 #7/#16「被交互捞回」）的误差棒能明确判定是否真>0。

### P2 — Owen value / 两级 Shapley（对应 Q3）
- [ ] 在 shapley 循环里把 `rng.shuffle(order)` 换成**分层洗牌**：先洗 section 顺序，
      再在每个 section 内洗 unit，保持每个 section 的 units 连续。marginal 记账不变。
      新增 `--method owen` 或 `--structured` 开关。
- [ ] 复用 `split_units(granularity="section")` 识别 section→unit 归属。
- [ ] 输出**两级结果**：section 级 Shapley（商博弈）+ 段内 unit 级。
- **验收**：与 flat-Shapley 对比排名是否更稳、方差是否下降、efficiency 仍 PASS。
