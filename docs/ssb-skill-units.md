...
3. **judge 在粗粒度筛选（triage）上够用、在细粒度归因上失效。** judge 把结构单元（`---`、标题）打低分、内容规则打高分，所以 judge-pruned 也删对了 boilerplate → 0.800。judge 的**失效是细粒度的**（见上文 Spearman −0.18/≈0、never-negative、认不出真正有害单元），而不是"选哪 25 个留下"这种粗活。
